...
class Product
  ...
  # take in the search term
  # format search term by adding percentage signs either side and then converting to lowercase
  # create the SQL query to return all products with name containg the search term
  # call our SQL Runner passing in the query and it's values
  # save all the results as an array of Products and return that array
...
# pda stuff delete after....................
  def self.sort_by_selling_price()
    sql = "SELECT selling_price FROM products ORDER BY selling_price DESC"
    results = SqlRunner.run(sql)
    return results.map { |product| Product.new(product) }
  end
# ...............up to here
